...
function getAll(req, res, next) {
    user_service_1.userServices.getAll()
        .then((users) => res.json(users))
        .catch(next);
}
function getById(req, res, next) {
    user_service_1.userServices.getById(Number(req.params.id))
        .then((user) => res.json(user))
        .catch(next);
}
function create(req, res, next) {
    user_service_1.userServices.create(req.body)
        .then(() => res.json({ message: 'User Created' }))
        .catch(next);
}
function update(req, res, next) {
    user_service_1.userServices.update(Number(req.params.id), req.body)
        .then(() => res.json({ message: 'User Updated' }))
        .catch(next);
}
function _delete(req, res, next) {
    user_service_1.userServices.delete(Number(req.params.id))
